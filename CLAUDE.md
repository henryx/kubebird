...
- The security database (`securityN.fdb`) is kept on the primary PVC (`/var/lib/firebird/data/securityN.fdb`, via `securityDatabasePath`) rather than the image's own ephemeral `/opt/firebird`. A `security-database-init` initContainer (same image, mounting the primary PVC, plus the backup PVC too when `spec.storage.backup` is set) runs before the `firebird` container and seeds it there, if it isn't already present — e.g. a reused primary PVC from an earlier `Instance` with the same name (see "Finalizer and deletion" below) already has one — from, in preference order: its own `gbak` backup left behind by `backupAndReleaseStorage` at `<backup mount>/base/securityN.fbk` (`instanceBackupDir`'s fixed `base` subdirectory — not named after the `Instance`, since `spec.storage.backup` is already a PVC dedicated to it; see "Backup-and-release on deletion" below), restored via a local `gbak -create -verify` (no host given, so it runs against the image's own local engine directly — the only option anyway, since no firebird server is listening yet at this point in the Pod's startup; confirmed against the actual image that this needs no `-user`/`-password`, since there's no pre-existing security database yet for it to authenticate the connection against), if `spec.storage.backup` is set and one exists there; otherwise the image's baked-in default (`securityDatabaseImageDefaultPath`), seeded with a plain `cp` since that source is a raw database file, not a `gbak` archive (`securityDatabaseInitScript`). The `firebird` container itself gets a `FIREBIRD_CONF_SecurityDatabase` env var (a documented image override) pointing the engine at that same relocated path.
...
A `kubebird.github.io/finalizer` finalizer lets Reconcile observe deletion, log it, and report `status.phase: Deleting` (also via `setPhase`) before removing the finalizer so Kubernetes' garbage collection of the owner-referenced objects (ConfigMap, Service, StatefulSet) proceeds. `setDeletionMessage` (skip-if-unchanged like `setPhase`, but deliberately never touching `status.error` since deletion isn't a reconcile failure) records what `reconcileDeletion` is currently doing into `status.message` at each step — "Deleting Instance" up front, "Removing finalizer" right before that final step, and (when `spec.storage.backup` is set) the `backupAndReleaseStorage` sub-steps below — so `kubectl get instances` reflects live deletion progress rather than a stale pre-deletion message. The primary/backup/shadow PVCs and the SYSDBA Secret are *not* owner-referenced — `reconcilePVC` and `reconcileSysdbaSecret` deliberately skip `controllerutil.SetControllerReference`, unlike every other resource Kubebird creates — so they're never part of that GC, meaning an `Instance`'s data and its SYSDBA credentials both survive deleting the `Instance` unless someone removes the PVCs or Secret directly.

#### 8. Backup-and-release on deletion

The one exception to PVCs surviving deletion is when `spec.storage.backup` is set: `reconcileDeletion` calls `backupAndReleaseStorage` before removing the finalizer, which first `mkdir -p`s `instanceBackupDir` — `<backup mount>/base`, a fixed subdirectory (not named after the `Instance`, since `spec.storage.backup` is already a PVC dedicated to it — naming the subdirectory after the `Instance` too would just stutter it into the path a second time) — then `exec`s `gbak -backup -verify` (via the same `execInPod` helper `reconcileDatabases` uses, renamed from `execIsql` since it's no longer isql-specific) for every entry in `status.databases`, writing `<database-without-.fdb>.fbk` into that directory. The security database is backed up the same way in principle, but can't be `exec`ed inline like the others: `gbak` backing up the security database while the `firebird` container still has it open live fails outright, confirmed against the actual image ("Database already opened with engine instance, incompatible with current" over a local connection, and "no permission for remote access to database" even via the services manager, `-se`) — the engine holds it exclusively while a server is attached to it, unlike regular application databases, which support the normal multi-attach `gbak` already relies on above. So instead, `backupAndReleaseStorage` scales the StatefulSet to 0 replicas first (stopping the pod and releasing that exclusive hold), waits for it to actually terminate, then creates a short-lived `<name>-security-backup` Pod (`createSecurityBackupPod`) that mounts the same primary and backup PVCs and runs a local `gbak -backup -verify` itself — no host given, so no server is involved at all, and confirmed against the actual image that this needs no `-user`/`-password` either, since a local backup of an already-existing database doesn't validate them against anything. `backupSecurityDatabaseOffline` polls that Pod (`Get`, since nothing here needs `List`/`Watch` for its own logic — see the RBAC note below) and deletes it once it reaches `Succeeded`. It then explicitly `r.Delete`s the primary PVC and, if set, the shadow PVC (the RBAC marker on `persistentvolumeclaims` includes `delete` for this) — the backup PVC itself is never touched.

- Every stage of this is driven by observable cluster state rather than a status field, so it's naturally idempotent across the repeated reconciles the StatefulSet scale-down and pod termination both need: `spec.Replicas` still non-zero means `spec.databases` hasn't been backed up yet, `spec.Replicas` zero but `status.Replicas` still non-zero means the pod hasn't fully stopped yet, and both zero means it's safe to back up the security database.
- Backing up `spec.databases` requires the StatefulSet's pod to be `Ready` (`reconcileDeletion` runs, and the owner-referenced StatefulSet isn't garbage collected, until the finalizer is removed, so the pod is still reachable at this point) — unconditionally now, even when `status.databases` is empty, since the security database always exists (seeded by `security-database-init` regardless of `spec.databases`) and always needs backing up before the primary PVC carrying it is released; the primary/shadow PVCs are only released without waiting on pod readiness when the StatefulSet doesn't exist at all (e.g. it was already garbage collected some other way).
- Each stage calls `setDeletionMessage`: `"Waiting for the Firebird pod to be ready before backing up databases"` if it has to block on that; `"Backing up databases into storage.backup"` right before the `gbak` step for `spec.databases`; `"Stopping the Firebird pod to back up the security database"` while scaling down and waiting for the pod to stop; `"Backing up the security database"` while the helper Pod runs; and `"Releasing primary and shadow storage"` right before the `r.Delete`s.
- The `pods` RBAC marker (`instance_controller.go`) grants `list`/`watch` too, even though `createSecurityBackupPod`/`backupSecurityDatabaseOffline` only ever call `Get`/`Create`/`Delete` on Pods directly: the manager's default client is a cached/informer-backed client, and the first `Get` or `Create` against a GVK it hasn't seen before starts a `List`+`Watch` informer for that whole type to populate its local cache — found the hard way, via `"pods is forbidden"` on `List` in the manager's own logs even with `get;create;delete` already granted.
- On a later recreate under the same name, this backed-up security database is what `security-database-init` restores (see "Core resources" above) instead of falling back to the image's stock default — the counterpart to `restoreDatabaseIfBackedUp` for `spec.databases`, but via a local `gbak -create` run directly by the init container rather than `reconcileDatabases`' `exec`ed one, since no firebird server is listening yet at that point for `exec` to reach.

### e2e coverage

Split across `test/e2e/instance_lifecycle_test.go`, `instance_pvc_reuse_test.go`, `instance_backup_orphan_test.go`, `instance_table_restore_test.go`, `instance_version_upgrade_test.go`, and `instance_security_database_test.go`, each wired into the `Describe("Manager", Ordered, ...)` block in `test/e2e/e2e_test.go` via its own `instance*Specs()` function (`instanceLifecycleSpecs()`, `instancePVCReuseSpecs()`, `instanceBackupOrphanSpecs()`, `instanceTableRestoreSpecs()`, `instanceVersionUpgradeSpecs()`, `instanceSecurityDatabaseSpecs()`), all called after CRDs/manager are deployed and before they're torn down.

- **`instanceLifecycleSpecs`** exercises the `Instance` CRD end to end against a real Kind cluster: CRD cross-field validation (rejects `shadow: true` without `storage.shadow`); full deployment (Secret/ConfigMap/Service/StatefulSet, the primary/backup/shadow PVCs including the backup volume's mount, plus real database and shadow files created inside the pod via `exec`, not just checked via status); adding a database to a running `Instance` without disturbing existing ones or restarting the pod; a rotated SYSDBA password instead restarting the pod (its start time changes) and authenticating with the new password once it's back up; and garbage collection on deletion — owned objects (ConfigMap, Service, StatefulSet) removed, the backup PVC always retained (never owner-referenced), the SYSDBA Secret also retained (never owner-referenced) rather than garbage collected, but the primary/shadow PVCs released instead (not just retained) once `storage.backup` is set, with a throwaway verification Pod confirming `gbak` actually wrote a `.fbk` file per database, plus a `gbak`-backed-up `securityN.fbk` for the security database itself, into the backup volume's fixed `base/` subdirectory before they're released. As one further `It` in the same Ordered `Instance` Context, right after that deletion, it also covers restoring both databases (plus the shadow file) from those `.fbk` backups, and the security database from its own backup via the `security-database-init` initContainer, when the identical CR is re-applied under the same name, asserting no reconcile error and the actual files present in the pod — plus, since the surviving Secret is reused as-is across the delete/recreate (rather than being regenerated), that the password read afterwards is unchanged from the one captured just before deletion and that SYSDBA actually authenticates against the restored database with it.
- **`instancePVCReuseSpecs`** covers the primary-PVC-reuse case specifically: provisions an `Instance` without `storage.backup`, deletes it (the primary PVC survives, unowned), re-applies the identical CR, and asserts the PVC is reused by UID (not recreated) and `status.databases`/`status.error` show the database was registered without a `CREATE DATABASE` failure.
...
- **`instanceSecurityDatabaseSpecs`** exercises the `security-database-init` init container's "leave it alone" path specifically: with `storage.backup` deliberately unset, a Firebird user created directly in the security database (not the SYSDBA account synced from the Secret) survives deleting and recreating the `Instance` under the same name — since the primary PVC (carrying the security database along with it) is unowned and survives deletion like in `instancePVCReuseSpecs`, and the init container only seeds a fresh default from the image when the file isn't already on it. Also confirms SYSDBA can still authenticate with the (unowned, and so unchanged) Secret's password against the reused security database (the image's own entrypoint re-applies `FIREBIRD_ROOT_PASSWORD` to whatever security database is present on every container start), and that `status.error` stays empty across the recreate.
