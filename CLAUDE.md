...
`test/e2e/instance_test.go`, wired into the `Describe("Manager", Ordered, ...)` block in `test/e2e/e2e_test.go` via `instanceLifecycleSpecs()`, `instancePVCReuseSpecs()`, `instanceBackupOrphanSpecs()`, `instanceTableRestoreSpecs()`, and `instanceVersionUpgradeSpecs()`, all called after CRDs/manager are deployed and before they're torn down.
