...
The one exception to PVCs surviving deletion is when `spec.storage.backup` is set: `reconcileDeletion` calls `backupAndReleaseStorage` before removing the finalizer, which first `mkdir -p`s `instanceBackupDir` — `<backup mount>/base`, a fixed subdirectory (not named after the `Instance`, since `spec.storage.backup` is already a PVC dedicated to it — naming the subdirectory after the `Instance` too would just stutter it into the path a second time) — then `exec`s `gbak -backup -verify` (via the same `execInPod` helper `reconcileDatabases` uses, renamed from `execIsql` since it's no longer isql-specific) for every entry in `status.databases`, writing `<database-without-.fdb>.fbk` into that directory, then (`backupSecurityDatabase`) a plain `cp` of the security database itself into that same directory as `securityN.fdb` — not a `gbak` archive, since `gbak` would need to authenticate against a security database to run, and this is that very file; a live filesystem copy isn't a transactionally consistent snapshot the way `gbak`'s is, accepted given how rarely the security database itself is written to. It then explicitly `r.Delete`s the primary PVC and, if set, the shadow PVC (the RBAC marker on `persistentvolumeclaims` includes `delete` for this) — the backup PVC itself is never touched.

- Backing up requires the StatefulSet's pod to be `Ready` (`reconcileDeletion` runs, and the owner-referenced StatefulSet isn't garbage collected, until the finalizer is removed, so the pod is still reachable at this point) — unconditionally now, even when `status.databases` is empty, since the security database always exists (seeded by `security-database-init` regardless of `spec.databases`) and always needs backing up before the primary PVC carrying it is released; the primary/shadow PVCs are only released without waiting on pod readiness when the StatefulSet doesn't exist at all (e.g. it was already garbage collected some other way).
- Each stage calls `setDeletionMessage`: `"Waiting for the Firebird pod to be ready before backing up databases"` if it has to block (returning an error too, so the finalizer stays and this reconcile retries), `"Backing up databases into storage.backup"` right before the `gbak`/`cp` steps, and `"Releasing primary and shadow storage"` right before the `r.Delete`s.
- On a later recreate under the same name, this backed-up security database is what `security-database-init` restores (see "Core resources" above) instead of falling back to the image's stock default — the counterpart to `restoreDatabaseIfBackedUp` for `spec.databases`, but via the init container's plain `cp` rather than `reconcileDatabases`' `gbak`, for the authentication-bootstrapping reason above.
