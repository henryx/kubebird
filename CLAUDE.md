...
Creates it (name from `spec.authentication.sysdba.secretRef`, defaulting to `<name>-sysdba` when unset) with a generated random password and a `username: SYSDBA` key if it doesn't exist yet, never touching one that already exists.
...
- The security database (`securityN.fdb`) is kept on the primary PVC (`/var/lib/firebird/data/securityN.fdb`, via `securityDatabasePath`) rather than the image's own ephemeral `/opt/firebird`. A `security-database-init` initContainer (same image, mounting only the primary PVC) runs before the `firebird` container and seeds it there from the image's baked-in default (`securityDatabaseImageDefaultPath`, via `securityDatabaseInitScript`) if it isn't already present — e.g. a reused primary PVC from an earlier `Instance` with the same name (see "Finalizer and deletion" below) already has one. The `firebird` container itself gets a `FIREBIRD_CONF_SecurityDatabase` env var (a documented image override) pointing the engine at that same relocated path.
...
Once the StatefulSet pod is ready, pushes the Secret's password to the live server whenever it drifts from `status.sysdbaPasswordHash` by `exec`ing `isql` directly against the security database file (e.g. `security3.fdb` for a `3.x` version, via the shared `securityDatabaseFileName` helper also used for the `security.db` alias above) rather than over the usual host:port connection — that embedded/local connection is trusted by OS user rather than by password, which sidesteps needing the password being replaced to authenticate. Firebird refuses a second engine instance — even this kind of embedded, OS-trusted one — on a database file another instance already has open, and the live server always has the security database open ("Database already opened with engine instance, incompatible with current"), so `execWithSecurityDatabaseOffline` briefly takes the server offline for this one connection instead of attaching alongside it: it `chmod -x`'s the `firebird` binary itself (so `fbguard` can't respawn it — killing `fbguard` too, to get a similar effect, would also kill the container, since it's what the entrypoint's foreground `wait` blocks on) and kills the running server, giving a deterministic offline window with nothing holding the file open, then restores the binary's executable bit and waits for `fbguard` to bring Firebird back up — and a real connection to succeed, not just the process to exist — before returning.
